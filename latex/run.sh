...
#docker exec -i --user user --workdir /home/user/letter/ latex-a ./run.sh
...
docker container exec -ti $name sed -i 's/^#MaxStartups.*/MaxStartups 100:100:100/g' /etc/ssh/sshd_config
docker container exec -ti $name sed -i 's/^#MaxSessions.*/MaxSessions 100/g' /etc/ssh/sshd_config
